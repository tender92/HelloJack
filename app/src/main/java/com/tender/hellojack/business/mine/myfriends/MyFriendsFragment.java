...
public class MyFriendsFragment extends BaseFragment implements MyFriendsContract.View {
    ...
    public void initUIData() {
        mContacts.clear();
        mFriends.clear();
        List<Friend> friends = mPresenter.getFriends();
        if (!StringUtil.isEmpty(friends)) {
            mFriends.addAll(friends);
        }
        if (mFriends.size() > 0) {
            for (int i = 0; i < mFriends.size(); i ++) {
                Friend friend = mFriends.get(i);
                UserInfo userInfo = mPresenter.getUserInfo(friend.account);
                mContacts.add(new Contact(friend, userInfo));
            }
            mPresenter.sortContacts(mContacts);
            if (tvFootView != null) {
            ...
            public void onLetterUpdate(String letter) {
                ...
                if ("↑".equals(letter)) {
                    ...
                    for (int i = 0; i < mContacts.size(); i ++) {
                        ...
                        String c = contact.mPinyin.charAt(0) + "";
    ...
    private void setAdapter() {
        mAdapter = new LQRAdapterForRecyclerView<Contact>(mActivity, R.layout.hj_layout_contact_item, mContacts) {
            ...
            public void convert(LQRViewHolderForRecyclerView helper, final Contact item, int position) {
                helper.setText(R.id.tv_my_friends_item_name, StringUtil.hasValue(item.mAlias) ? item.mAlias : item.mName);
                if (StringUtil.hasValue(item.mAvatar)) {
                    ImageLoaderUtil.loadLocalImage(item.mAvatar, (ImageView) helper.getView(R.id.iv_my_friends_item_header));
                } else {
                    helper.setImageResource(R.id.iv_my_friends_item_header, R.mipmap.hj_mine_default_header);
                }

                String str = "";
                String currentLetter = item.mPinyin.charAt(0) + "";//得到当前字母
                if (position == 0) {
                    str = currentLetter;
                } else {
                    String preLetter = mContacts.get(position - 1).mPinyin.charAt(0) + "";//得到上一个字母
                    ...
                    if (netIndex < mContacts.size() - 1) {
                        String netLetter = mContacts.get(position + 1).mPinyin.charAt(0) + "";//得到下一个字母
                ...
                RxView.clicks(helper.getView(R.id.ll_my_friends_item)).throttleFirst(1, TimeUnit.SECONDS).observeOn(ScheduleProvider.getInstance().ui()).subscribe(new Action1<Void>() {
                    ...
                    public void call(Void aVoid) {
                        ...
                        intent.putExtra(IntentConst.IRParam.MY_FRIENDS_ACCOUNT, item.mAccount);
